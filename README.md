...
Mailtrain は、Node.js (v14+) と、MySQL (v8+) または MariaDB (v10+) で構成されたオンプレミスのニュースレター配信アプリケーションです。

こちらは Mailtrain バージョン 2 です。v1の機能をほぼすべて実装し、さらにいくつかの機能を追加しています。完全に書き直したので、一からインストールする必要があります。

Mailtrain v1 からアップグレードする場合は、データベースをバックアップし、そのバックアップを Mailtrain v2 で使用してください。Mailtrain v2 で、データベースを新しいスキーマにアップグレードできるはずです。
...
* リストのセグメント化
* 独自フィールド
* メールテンプレート (MJML ベース テンプレートを含む)
* 独自レポート
* 自動化 (トリガーと RSS キャンペーン)
* きめ細かなユーザー権限と柔軟な共有機能を備えたマルチユーザー環境
* エンタープライズ水準環境における階層型名前空間
* メール配信について「ほぼゼロ設定」を実現する内蔵 Zone-MTA (https://github.com/zone-eu/zone-mta)
...
### 準備
Mailtrainは、「trusted」・「sandbox」・「public」と呼ばれる 3 つの URL エンドポイントを作成します。これにより、Mailtrain
は、マルチユーザー環境におけるセキュリティを確保し、XSS 攻撃を防ぐことができます。これら 3 つのエンドポイントの機能は以下の通り :
- *trusted* - ログインしたユーザーがリストの管理やキャンペーンの送信などを行う UI のメイン画面です。
- *sandbox* - ユーザーからは直接見えないエンドポイントです。WYSIWYG テンプレートエディタのホストとして使用します。
- *public* - 読者向けのエンドポイントです。配信登録管理フォーム、ファイル、およびアーカイブをホストするために使用します。

Mailtrain の推奨配置では、すべて**同じ** IP アドレスを指す 3 つの DNS エントリーを作ります。例えば以下の通り:
- *lists.example.com* - public エンドポイント (`example.com` ドメイン下の `lists` A レコード)
- *mailtrain.example.com* - trusted エンドポイント (`example.com` ドメイン下の `mailtrain` CNAME レコードで `lists` を指す)
- *sbox-mailtrain.example.com* - sandbox エンドポイント (`example.com` ドメイン下の `sbox-mailtrain` CNAME レコードで `lists` を指す)


### クリーンインストールした CentOS 7 または Ubuntu 18.04 LTS へのインストール (public web サイトは SSL で暗号化)
